  ...
  return (
    <div className={`w-full h-screen overflow-y-scroll no-scrollbar pt-32 pb-10 sm:px-24 px-2 ${darkMode?"bg-slate-900":"bg-slate-200"}`}>
      ...
      <div onClick={save?editNote:saveNote} className="absolute lg:top-24 bottom-20 right-16">
        <ButtonIcon Icon={save?FaEdit:IoMdSave} action={save?'Edit':'Save'} className={'relative p-2 bg-gray-400 rounded-full hover:bg-gray-300 hover:scale-110 transition-all duration-300 hover:shadow-xl shadow-black group'} iconClassName={`text-xl`} actionClassName={'hidden lg:block absolute -left-4 -bottom-9 bg-gray-600 text-slate-300 px-5 rounded-full font-semibold scale-0 group-hover:scale-100 transition-all duration-300'} />
      </div>
    </div>
  )
}
