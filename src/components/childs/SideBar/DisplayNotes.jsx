    ...
    <>
      {
        allNotes.map((note)=>(
          ...
          <ButtonIcon
            ...
            action={note.title.length >= 10 ? note.title.substring(0, note.title.lastIndexOf(" ", 10)) + "...." : note.title}
            className={`flex items-center gap-4 my-2`}
            iconClassName={`${darkMode ? "text-slate-50" : "text-slate-900"} lg:text-xl sm:text-lg text-sm`}
            actionClassName={`${darkMode ? "text-slate-50" : "text-slate-900"} lg:text-lg sm:text-md text-sm capitalize`}
          />
